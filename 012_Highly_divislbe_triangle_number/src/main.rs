...
fn find_prime_occurences (number : i64, prime:&i64) -> (i64, i64) {
	let mut occurences = 0;
	let mut non_divisible_number = number;
	loop {
		if non_divisible_number % *prime == 0 {
			occurences += 1;
			non_divisible_number = non_divisible_number / *prime;
		} else {
			break;
		}
	}
...
fn main() {
	...
	for n in 1.. {
		...
		let (mut prime_a, mut prime_b) = (vec::<[i64;2]>![], vec::<[i64;2]>![]);
		...
		let mut sums_prime_occurences = vec::<i64>![];
